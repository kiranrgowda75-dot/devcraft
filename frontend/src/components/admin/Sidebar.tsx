...
import { X, LayoutDashboard, FolderKanban, Tag, TrendingUp, Settings, Plus, HelpCircle, LogOut } from 'lucide-react';
...
export default function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 z-40 bg-black/40 md:hidden backdrop-blur-sm animate-fade-in"
          onClick={onClose}
        />
      )}
      ...
      {/* Sidebar Aside */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 flex flex-col h-full transition-transform duration-300 md:static md:translate-x-0 ${
      ...
      >
        ...
        <div className="p-4 border-t border-gray-100 space-y-4">
          ...
          <div className="space-y-1">
            <Link href="/contact" onClick={onClose} className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-gray-500 hover:text-navy-900 transition-colors">
            ...
            <button 
              ...
              className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium text-gray-500 hover:text-red-600 transition-colors text-left"
